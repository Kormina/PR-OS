...
namespace HeatClassLibrary
{
    public class Heat
    {
        ...
        public double C; //средняя теплоёмкость воздуха при 200 градусах кДж/(м3 • К)
        public double Gm; //Расход окатышей (кг/с)
        public double Cm; //теплоёмкость окатышей (кДж/(кг • К))
        public double d; //диаметр аппарата (м)
        public double a; //Объемный коэффициент теплоотдачи (Вт/(м3 • К)) 
        public
        ...
        public HeatOut calc() // Все расчеты выполнены аналогично формулам из ПР1, более подробно можно ознакомиться в Excel файле ПР1
        {
            ...
            return new HeatOut
            {
                ...
                coordinates = x.GetRange(0, x.Count),
